...
export default function RouteOverviewDialog() {
  ...
  const handleShareLink = async () => {
    if (!share) return;
    const text = `Open this route in ProYard Sales Map: ${share.url}\n(or paste code ${share.code})`;
    // Prefer native share on mobile
    if (typeof navigator !== 'undefined' && (navigator as Navigator).share) {
      try {
        await (navigator as Navigator).share({ title: 'ProYard Sales Map', text, url: share.url });
        return;
      } catch {
    ...
    a.href = `mailto:?subject=${encodeURIComponent('ProYard Sales Map')}&body=${encodeURIComponent(text)}`;
    a.rel = 'noopener';
    a.click();
  };

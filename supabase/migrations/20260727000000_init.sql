...
returns boolean as $$
  select coalesce(
    (auth.jwt() ->> 'email') ~* '@(proyardservices\.com|getrudiment\.com)$',
    false
  );
...
create table if not exists accounts (
  ...
  -- Services (subset of mowing/fertilizer/pestControl/sprinklers).
  -- "Full Service" is derived client-side when all 4 are present, not stored.
  ...
  -- Two distinct addresses: QB "Bill to" vs "Ship to" (the job site the rep drives to)
  ...
  -- Contact — full QB contact shape preserved
  salutation text,
  ...
  -- account, untouched — the "don't lose any data" guarantee.
  qb_raw_rows jsonb not null default '[]',

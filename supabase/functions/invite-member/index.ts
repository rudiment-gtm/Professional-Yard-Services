...
// Requires a signed-in caller (verify_jwt stays on); the actual invite send
// needs the service-role client since admin.inviteUserByEmail is a
// privileged Auth Admin API call.
...
// Same allowed domains as validate-email-domain — keep both in sync.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!ALLOWED_DOMAINS.includes(domain)) {
      return new Response(JSON.stringify({ error: `Only @${ALLOWED_DOMAINS.join(' or @')} emails can be invited` }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }

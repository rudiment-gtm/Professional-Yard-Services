...
// Called by the `accounts_canceled_notify` DB trigger (via pg_net) whenever
// an account's status flips into 'canceled' from any other status. Fetches
// the full current row and POSTs it to a separate n8n webhook — set as its
// own Edge Function secret so it isn't hardcoded in migration SQL:
//   supabase secrets set N8N_STATUS_CANCELED_WEBHOOK_URL=https://rudiment.app.n8n.cloud/webhook/status/cancelled
  ...
  const logAttempt = async (status: "ok" | "error", extra: Record<string, unknown>) => {
    try {
      await admin.from("hubspot_sync_log").insert({
        ...
        action: "n8n_to_canceled",
        status,
        ...extra,
      });
    } catch (logErr) {
      console.error("[notify-n8n-status-canceled] failed to write sync log", logErr);
    }
  };

  try {
    const webhookUrl = Deno.env.get("N8N_STATUS_CANCELED_WEBHOOK_URL");
    if (!webhookUrl) {
      console.error("[notify-n8n-status-canceled] N8N_STATUS_CANCELED_WEBHOOK_URL is not configured");
    ...
    console.error("[notify-n8n-status-canceled] uncaught", msg);
